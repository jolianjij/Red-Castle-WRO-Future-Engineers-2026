...
The control block, in order, is:

    dir = Err * kp                      # SIGN steering is the PRIMARY control
    if outer wall too close: dir = +-45 # wall override (this also turns corners)
    lines -> driving direction and quadrant count

ONE DELIBERATE DEPARTURE from a pure sign/wall-override design: a pure-override
design has no lane keeping - with no sign in view, Err = 0 and the car drives
STRAIGHT, relying on the wall override to fire often enough to stay in the
corridor. That does not hold on our optics: measured L=0.112 R=0.129 against our
0.213 override threshold, so 44% of a run was steer = 0 and the car drove into
things. Between signs we therefore fall back to the outer-wall law that scored
full marks in the Open Challenge. Sign steering is still primary and the override
...
WHAT IS OURS:
  * every distance/threshold is MEASURED on our own camera, not borrowed from a
    published reference - a borrowed number does not transfer between optics
    (proven when a reference open-challenge constant read 0.021 against its own
    0.5 setpoint once measured on our camera).
  * the corrected wall mask, so the blue/orange lines and mat markings are not
    counted as walls.
  * MAGENTA COUNTS AS A WALL until the parking phase; we have no parking walls to
    tune against yet, so it stays a wall for the whole run.
...
PILLAR_KP = 0.111       # reference gain was 0.25 against a +-45 clamp; we clamp at
                        # +-20, so 0.25 * (20/45) keeps the same response shape
PILLAR_MIN_AREA = 60    # min contour area in our 320x160 buffer
SIGN_MEMORY = 8         # frames to hold a manoeuvre after losing the sign
Y_SCALE = 120.0 / R.PROC_H   # the reference frame was 120 rows tall, ours is 160
MAX_RUNTIME_S = 150     # SAFETY net only, not a lap limit
STOP_ON_LAPS = True     # stop automatically once the lap counter is satisfied
DEBUG = True


def find_sign(hsv):
    """Largest red/green contour that is TALLER than wide.
    Returns (kind, cx, cy, area) using the centroid, or None."""
    best = None
...
def sign_error(kind, cx, cy):
    """Sign-positioning error, with y mapped onto the reference 120-row frame."""
    y = cy * Y_SCALE
    if kind == "green":
...
def main():
    ...
    print(f"OBSTACLE - sign-priority + wall-override structure, our measured constants")
    ...
    try:
        ...
        while True:
            ...
            if sign is not None:
                ...
                # NOT "drive straight". A design that leans on the wall override
                # firing constantly does not hold on our optics: measured L=0.112
                # R=0.129 against a 0.213 threshold, so 44% of a run was steer=0
                # and the car simply drove into things. Between signs we use the
                # outer-wall law that scored full marks in the Open Challenge.
